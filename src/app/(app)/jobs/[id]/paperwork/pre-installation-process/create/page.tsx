...
export default function PreInstallationProcessCreatePage() {
  ...
  async function onSubmit(data: z.infer<typeof tmpCheckingProcessSchema>) {
    ...
    try {
        const payload = {
            ...data,
            locationDetails: { ...data.locationDetails, comment: data.locationDetails.comment || '' },
            shape: { ...data.shape, comment: data.shape.comment || '' },
            directionAndProtection: { ...data.directionAndProtection, comment: data.directionAndProtection.comment || '' },
            requiredSpeedRestrictions: { ...data.requiredSpeedRestrictions, comment: data.requiredSpeedRestrictions.comment || '' },
            plantAndEquipment: { ...data.plantAndEquipment, comment: data.plantAndEquipment.comment || '' },
            personalSafety: { ...data.personalSafety, comment: data.personalSafety.comment || '' },
            layoutDiagrams: { ...data.layoutDiagrams, comment: data.layoutDiagrams.comment || '' },
        ...
        router.push(`/jobs/${jobId}/paperwork/pre-installation-process`);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <CardFooter className="justify-end gap-2">
            <Button type="button" variant="ghost" onClick={() => router.push(`/jobs/${jobId}/paperwork/pre-installation-process`)}>Cancel</Button>
